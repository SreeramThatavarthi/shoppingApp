  ...
  const imagePickerr = async e => {
    ...
    const imagePicker = async e => {
      ...
      const addItem = async () => {
        ...
        try {
          firebase.database().ref("necklace/"+v4()).set(
            {
 ...
 const deleteItem=(id)=>{
   ...
   .catch((err)=>console.log(err))
      ...
      return(
          <div>
          {
              admin==true?(<div>
                ...
                <div className="form-group">
                    <label for="image" style={{marginRight:"10px"}}>Image 1</label>
                    <input
                    type="file"
                    name="image"
                    id="imagepicker"
        ...
        <Fragment>
          {
            (necklace!=null) ?(Object.entries(necklace).map(([key,value])=>(
