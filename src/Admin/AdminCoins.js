  ...
  const imagePickerr = async e => {
    ...
    const imagePicker = async e => {
      ...
      const addItem =  () => {
        ...
        try {
          firebase.database().ref("coins/"+v4()).set(
            {
              name,weight,wastage,cost,mc,del,picture:downloadUrl,picturee:downloadUrll
            }
          )
        } catch (error) {
          console.log(error)
      ...
      const getData=async ()=>{
        const ringsref=await firebase.database().ref('/coins');
   ...
   const deleteItem=(id)=>{
     ...
     .catch((err)=>console.log(err))
      ...
      return(
          ...
          <Fragment>
          {
            (coins!=null) ?(Object.entries(coins).map(([key,value])=>(
