...
const AdminStirs=()=>{
  ...
  const imagePickerr = async e => {
    ...
    try{
      ...
      error=>{
        toast("Something is wrong in change state");
      },
      ()=>{
    ...
    const imagePicker = async e => {
      ...
      const addItem = async () => {
        ...
        try {
          firebase.database().ref("stirs/"+v4()).set(
            {
   ...
   const deleteItem=(id)=>{
     ...
     .catch((err)=>console.log(err))
      ...
      return(
          ...
          <Fragment>
          {
            (stirs!=null) ?(Object.entries(stirs).map(([key,value])=>(
