  ...
  const imagePickerr = async e => {
    ...
    const imagePicker = async e => {
      ...
      const addItem =  () => {
        ...
        if(kdm==true)
        {
        var g=parseFloat(silverCost)+75;
        var cost=(parseFloat(weight) +  parseFloat(wastage))*parseFloat(g)+parseFloat(mc)
        }
        else{
        var cost=(parseFloat(weight) +  parseFloat(wastage))*parseFloat(silverCost)+parseFloat(mc);
        }
        try {
          firebase.database().ref("silver/"+v4()).set(
            {
   ...
   const deleteItem=(id)=>{
     ...
     .catch((err)=>console.log(err))
      ...
      return(
          ...
          <Fragment>
          {
            (silver!=null) ?(Object.entries(silver).map(([key,value])=>(
