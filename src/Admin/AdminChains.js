  ...
  const imagePickerr = async e => {
    ...
    const imagePicker = async e => {
      ...
      const addItem =  () => {
        ...
        try {
          firebase.database().ref("chains/"+v4()).set(
            {
      ...
      const getData=async ()=>{
        const ringsref=await firebase.database().ref('/chains');
   ...
   const deleteItem=(id)=>{
     ...
     .catch((err)=>console.log(err))
      ...
      return(
          ...
          <Fragment>
          {
            (chains!=null) ?(Object.entries(chains).map(([key,value])=>(
